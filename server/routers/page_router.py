...
from forms.get_all_clients import GetAllClientsForm
from forms.get_all_orders import GetAllOrdersForm
from forms.delete_client import DeleteClientForm
from forms.put_client import PutClientForm
from forms.put_order import PutOrderForm
...
@router.route("/get_all_clients")
@login_required
def get_all_clients():
    get_all_clients_form = GetAllClientsForm()
    return render_template('get_all_clients.html', form=get_all_clients_form)


@router.route("/get_all_orders")
@login_required
def get_all_orders():
    get_all_orders_form = GetAllOrdersForm()
    return render_template('get_all_orders.html', form=get_all_orders_form)
...
def put_client():
    ...
    if put_client_form.validate_on_submit():
        ...
        session.close()
        return redirect("/")
    return render_template("put_client.html", title="Изменение клиента", form=put_client_form)
...
def put_order():
    ...
    if put_order_form.validate_on_submit():
        ...
        session.close()
        return redirect("/")
    return render_template("put_order.html", title="Изменение клиента", form=put_order_form)
